...
function vtkResliceCursorLineRepresentation(publicAPI, model) {
  ...
  publicAPI.complexWidgetInteraction = (displayPosition) => {
    ...
    if (model.interactionState === InteractionState.ON_CENTER) {
      const intersectionPos = publicAPI.getProjectionOnPlane(displayPosition);
  ...
  publicAPI.getActors = () => {
    ...
    model.renderer.getActiveCamera().modified();

    // Don't clip away any part of the data.
    model.renderer.resetCameraClippingRange();
    //
    // console.log(
    //   model.renderer.getActiveCamera().getPosition(),
    //   model.renderer.getActiveCamera().getFocalPoint(),
    //   normalPlane.getNormal(),
    //   model.planeSource.getNormal(),
    //   model.resliceAxes
    // );
